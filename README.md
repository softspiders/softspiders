...
Известным подходом к оптимизации затрат, о котором часто упоминается в рассказах о стартапах, может быть потому, что о
стартапах вообще часто говорят, является разделение разработки на этапы по схеме: *проверка концепции* - *Proof of
Concept (PoC)*, создание *прототипа*, получение *минимально жизнеспособного продукта* - *Minimum Viable Product (MVP)*.  
...
Но каждый раз действует правило **минимально достаточного вложения сил**, при котором вложения должны быть соразмерны
задаче каждого этапа, но не более того.
